...
public class SalesAdapter extends ArrayAdapter<Item> {
    ...
    public SalesAdapter(Context context, List<Item> dataList){
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (itemView == null) {
            itemView = LayoutInflater.from(getContext()).inflate(R.layout.sales_items, parent, false);
        }
        ...
        if (data != null) {
            ...
            TextView profit = (TextView) itemView.findViewById(R.id.profit);
            TextView days = (TextView) itemView.findViewById(R.id.days);

            id.setText("상품명 : "+data.getId());
            if(data.getPrice().equals("share item")){
                profit.setText("이익 : 0 원"); // endPrice로 변경해줘야함
                price.setText("무료 나눔");
            }else{
                Integer profitValue = (Integer.parseInt(data.getEndPrice())/20);
                profit.setText("이익 : " + String.valueOf(profitValue) + " 원"); // endPrice로 변경해줘야함
                price.setText("최종가 : "+String.valueOf(data.getEndPrice())+" 원");
            }
